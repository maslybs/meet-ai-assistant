...
- `browse_web_page` – тул для завантаження веб-сторінок. Повертає Markdown з посиланнями, що дозволяє користувачу переходити по них у наступних запитах.
  - **Jina Reader (основний метод)** – безкоштовний API (1000 запитів/день), обходить більшість блокувань. Налаштування:
    - `VOICE_AGENT_JINA_ENABLED` – увімкнути/вимкнути Jina (за замовчуванням `1`)
    - `JINA_API_KEY` – опційний ключ для збільшення лімітів
    - `VOICE_AGENT_JINA_TIMEOUT` – таймаут запиту (за замовчуванням 30 сек)
  - **Playwright (fallback)** – headless Chromium, використовується якщо Jina недоступна. Налаштування: `VOICE_AGENT_PLAYWRIGHT_FALLBACK`, `VOICE_AGENT_BROWSER_HOME`, `VOICE_AGENT_BROWSER_TIMEOUT_MS`, `VOICE_AGENT_BROWSER_MAX_CHARS`, `VOICE_AGENT_BROWSER_USER_AGENT`, `VOICE_AGENT_BROWSER_LOCALE`, `VOICE_AGENT_BROWSER_TIMEZONE`, `VOICE_AGENT_BROWSER_WAIT_UNTIL`, `VOICE_AGENT_BROWSER_CHROMIUM_ARGS`, `VOICE_AGENT_BROWSER_VIEWPORT_WIDTH`, `VOICE_AGENT_BROWSER_VIEWPORT_HEIGHT`, `VOICE_AGENT_BROWSER_EXTRA_WAIT_MS`, `VOICE_AGENT_BROWSER_IDLE_SECONDS`, `VOICE_AGENT_BROWSER_ENABLE_PROXY`.
- **Webshare auto-proxy (для Playwright fallback)** – якщо потрібно маскуватися під різні IP, задайте `VOICE_AGENT_WEBSHARE_API_KEY`, опційно `VOICE_AGENT_WEBSHARE_QUERY` та `VOICE_AGENT_WEBSHARE_TIMEOUT`. Для статичного проксі задайте `VOICE_AGENT_BROWSER_PROXY_SERVER/USERNAME/PASSWORD`. Щоб вимкнути проксі, поставте `VOICE_AGENT_BROWSER_ENABLE_PROXY=0`.
...
- `fetch_rss_news` – тул для читання RSS. Список доступних категорій та їх URL винесено у файл `voice_agent/data/rss_feeds.json` (можна замінити через `VOICE_AGENT_RSS_CATALOG_FILE`). Асистент перед викликом інструмента озвучує категорії з цього каталогу і підставляє відповідний URL або `id`. Аргумент `feed_url` обов'язковий: якщо його не передати, тул поверне інструкцію з переліком стрічок. Ліміт публікацій задається аргументом `limit` чи `VOICE_AGENT_RSS_LIMIT`; `VOICE_AGENT_RSS_USER_AGENT` визначає HTTP User-Agent.
- `VOICE_AGENT_RSS_CATALOG_FILE` – шлях до користувацького JSON з переліком RSS (формат такий самий, як у `voice_agent/data/rss_feeds.json`). Дозволяє додавати нові сайти без змін у коді.
- `list_radio_feeds`, `get_radio_episodes`, `search_radio_episodes`, `play_radio_episode`, `play_audio_url`, `stop_radio_playback`, `radio_playback_status` – туди додано radio/RSS playback. Каталог лежить у `voice_agent/data/radio_feeds.json` або задається через `VOICE_AGENT_RADIO_CATALOG_FILE`. Для `ukr-radio.bgdn.dev` скопіюй RSS URL каналу/програми з `https://ukr-radio.bgdn.dev/programs/` або власну стрічку з `/builder/` і встав у поле `url`. Програвання йде через окремий LiveKit audio track та `ffmpeg`; на сервері має бути встановлений `ffmpeg`, або задай шлях у `VOICE_AGENT_FFMPEG_BIN`.
